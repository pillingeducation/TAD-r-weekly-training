...
4.  **`mutate()`**
    ...
    4.1 Create a flag for flights that departure was delayed using `mutate()` and `if_else()`.
...
7. **`pivot_wider() and pivot_longer()**
    ...
    7.1 Read the [R for Data
    Science section 12](https://r4ds.had.co.nz/tidy-data.html?q=pivot#tidy-data)
