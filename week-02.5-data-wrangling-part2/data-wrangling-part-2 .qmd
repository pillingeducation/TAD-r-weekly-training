...
`mutate()` can be used with operations (*, +, etc) and is often used with `if_else()` or `case_when()`
...
`group_by()` is a very useful function that can be used for data aggregation.
...
There are a number of different joins see [R for Data Science: Relational data](https://r4ds.had.co.nz/relational-data.html?q=join#relational-data)

I mainly use `left_join()`

With `left_join()` You keep all variables in dataset `x` and though  
